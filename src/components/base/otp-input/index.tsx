...
const OtpItem: FC<IOtpItem> & FunctionComponent<IOtpItem> = ({ index }: IOtpItem) => {
  const {
    inputRef,
    onPress,
    otpValue,
  ...
  const animations = ANIMATION_VARIATIONS[animationVariant ?? 'fadeSlideDown'];
...
export const OtpInput: FC<IOtpInput> & FunctionComponent<IOtpInput> = memo<IOtpInput>(
  ({
    ...
    const inputProps: IOtpContext = {
      ...
      otpCount,
      editable,
      autoFocus: autoFocus ?? enableAutoFocus,
      enteringAnimated,
      exitingAnimated,
      error,
      inputBorderRadius,
